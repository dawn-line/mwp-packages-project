...
import { User, Sunny, Bell, SwitchButton, Setting } from '@element-plus/icons-vue'
...
const compare = (rule: any, value: any, callback: any) => {
...
const changePwd = async function (formEl: FormInstance | undefined) {
  if (!formEl) return
...
const openSystem = async function (item: object) {
  if (item.appEntrance) {
    // 请求跳转应用的默认模块
    const result = await $http().get(`/metaServer/module/default-module?applicationId=${item.id}`)
    if (result.data && result.data.status === 'success') {
      const { productCode, moduleUrl, moduleId, orgId } = result.data.result
...
const getAppList = async function () {
  const result = await $http().get('/metaServer/application/user-apps')
  if (result.data && result.data.status === 'success') {
    applicationList.push(...result.data.result)
  } else {
    console.log(result)
  }
}
