<template>
  <div class="login">
    <div class="content">
      ...
      <P class="english">
        Welcome to the Operation management platform
      </P>
    </div>
    <div class="login-form">
      ...
      <div class="btn-line">
        <div class="btn pwd" @click="changeLoginType">{{ loginText }}</div>
...
import { $http } from '../../asset/http'
import { md5, getUrlParams } from '../../asset/utils'
const { onMounted, reactive, ref } = Vue
const { FormInstance, FormRules, ElMessage } = ElementPlus
...
const getCode = async function () {
  const result = await $http().get('/cas/verifyCode')
  if (result.data && result.data.status === 'success') {
    const _code = result.data.result
    code.img = _code.img
...
const getVCode = async function () {
  ...
  const result = await $http().get(`/cas/smsVerifyCode?phoneNumber=${loginCodeForm.phoneNumber}&flag=loginCode`)
  if (result.data.status === 'success' && (result.data.result.code === 'OK' || result.data.result.code === 'success')) {
    isVarify.value = true
...
const _login = async function (formEl: FormInstance | undefined) {
  if (!formEl) return
    ...
    if (valid) {
      try {
           if (loginType.value) {
            ...
            if (result.data && result.data.status === 'success') {
              sessionStorage.setItem('__cstk', result.data.result.access_token)
              ElMessage({
                message: '登入系统成功！',
                type: 'success'
              })
...
const beian = function () {
  window.open('https://beian.miit.gov.cn/#/Integrated/index')
}
const changeLoginType = function() {
